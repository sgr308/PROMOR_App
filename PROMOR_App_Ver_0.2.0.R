                        ...
                        tabItem(tabName = "home",
                                fluidRow(
                                  box(
                                    ...
                                    tags$ul(
                                      tags$li(p(tags$b("S. Patel et al., PROMOR App: A web application for label-free quantification (LFQ) proteomics data analysis and predictive modeling, 2022 IEEE International Conference on Bioinformatics and Biomedicine (BIBM), Las Vegas, NV, USA, 2022, pp. 3864-3866, doi: 10.1109/BIBM55620.2022.9995277."))),
                                      tags$li(p(tags$b("Ranathunge, C. et al., promor: a comprehensive r package for label-free proteomics data analysis and predictive modeling. bioRxiv 2022.08.17.503867; doi: https://doi.org/10.1101/2022.08.17.503867"))),
                                      ),
                                    h3("Contact Us"),
  ...
  observeEvent(input$subm, {
   ...
   create_df2 <- function(prot_groups = input$file1$datapath,
    ...
    output$dqim7 <- renderPlot({
      impute_plot(original = raw_filtered, imputed = df, global = input$dqim1, n_row = input$dqim2, n_col = input$dqim3, text_size = input$dqim4, dpi = input$dqim5, palette = input$dqim6)})
    ...
    output$dqim9 <- downloadHandler(
      ...
      content = function(file){
        if(input$dqim8 == "png")
          ...
          pdf(file) # open the pdf device
        print(impute_plot(original = raw_filtered, imputed = df, global = input$dqim1, n_row = input$dqim2, n_col = input$dqim3, text_size = input$dqim4, dpi = input$dqim5, palette = input$dqim6))
        dev.off()
      })
    ...
    output$dqn5 <- renderPlot({
      norm_plot(original = df, normalized = df, type = input$dqn1, text_size = input$dqn2, dpi = input$dqn3, palette = input$dqn4)})
    ...
    output$dqn7 <- downloadHandler(
      ...
      content = function(file){
        if(input$dqn6 == "png")
          ...
          pdf(file) # open the pdf device
        print(norm_plot(original = df, normalized = df, type = input$dqn1, text_size = input$dqn2, dpi = input$dqn3, palette = input$dqn4))
        dev.off()
      })
    ...
    output$rhp5 <- renderPlot({
      heatmap_de(fit_df, df, adj_method = input$dex3, cutoff = input$dex1, lfc = input$dex2, n_top = input$dex6, sig = input$rhp1, text_size = input$rhp2, dpi = input$rhp3, palette = input$rhp4)})
    ...
    output$rhp7 <- downloadHandler(
      ...
      content = function(file){
        if(input$rhp6 == "png")
          ...
          pdf(file) # open the pdf device
        print(heatmap_de(fit_df, df, adj_method = input$dex3, cutoff = input$dex1, lfc = input$dex2, n_top = input$dex6, sig = input$rhp1, text_size = input$rhp2, dpi = input$rhp3, palette = input$rhp4))
        dev.off()
      })
